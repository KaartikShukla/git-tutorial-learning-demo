...
if(isset($_GET['type']) && $_GET['type']!='')
{
  ...
  if($type='status')
  {
    ...
    $update_status="UPDATE product SET productstatus='$status' WHERE productid='$id'";
...
<head>
  ...
  <title>AdminLTE 3 | View Orders Elements</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <div class="container">
            <div class="container-fluid bg-primary text-white text-center rounded"style=" margin-bottom:30px;"!important><h1 style="font-size: 50px;">View Orders</h1>
            </div>
                    <?PHP if(isset($delete_fail))
                    {?>
            ...
            <div>
                <table class="table-striped text-center" style="width:100%";>
                    ...
                    if(isset($_SESSION['level']))
                    {
                        ...
                        <tr align="center">
                        <td><?php echo $c++;?></td>
                        <td><img src="<?php echo $fetch['image']?>" style="height: 50px;"></td>
                        <td><?php echo $fetch['productname']?></td>
                        <td><?php
                            $category_sql="SELECT categoryname FROM category where categoryid='".$fetch['categoryid']."'";
                              $res=mysqli_query($con,$category_sql);
                                $row=mysqli_fetch_assoc($res);
                            echo $row['categoryname']?></td>
                        <td><?php echo $fetch['productmrp']?></td>
                        <td><?php echo $fetch['productprice']?></td>
                        <td><?php echo $fetch['quantity']?></td>
                        <td><?php if($fetch['productstatus']==1){?>
                          <a href="?type=status&operation=deactive&id=<?php echo $fetch['productid'];?>"><button class="btn btn-success">Undelivered</button>
                        <?php }
                        else{ ?><a href="?type=status&operation=active&id=<?php echo $fetch['productid'];?>"><button class="btn btn-danger">Delivered</button> <?php } ?> </td>
                        <td><a href="productinfo.php?id=<?php echo $fetch['productid'];?>"><button class="btn btn-primary">View</button></td>
                        <td><a href="updateproduct.php?id=<?php echo $fetch['productid'];?>"><button class="btn btn-warning">Update</button></td>
                        <td><a href="deleteproduct.php?id=<?php echo $fetch['productid'];?>" onclick="return confirm('Are you sure?')"><button class="btn btn-danger">Delete</button></td>
                        </tr><?php
                        }
                    }
                    else
